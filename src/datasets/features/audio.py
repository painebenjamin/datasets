...
class Audio:
    ...
    def embed_storage(self, storage: pa.StructArray) -> pa.StructArray:
        ...
        if isinstance(bytes_array, pa.ChunkedArray):
            # This is a rare case where the audio files overflow the binary container. We only take the first chunk for now.
            bytes_array = bytes_array.chunks[0]
            num_included = len(bytes_array)
            num_excluded = len(paths_list) - num_included
            warnings.warn(
                f"{num_excluded} audio files are not included in the Arrow array because they are larger than expected. " \
